function [explanation, model] = run_dbsherlock(dataset, abnormalIdx, normalIdx, modelName, exp_param)
    ...
    if ~goto_evaluation
        % Original predicate generation code
        predicates = {};
    ...
    % At the start of the function, before any other processing
    if exp_param.use_llm_rules && ~isempty(exp_param.current_rules)
        predicates = exp_param.current_rules;
        return;  % Skip regular predicate generation
    end

    % Generate LLM rules first if enabled
    if use_llm_rules
        try
            [llm_rules, llm_predicates] = llm_rule_generator(dataset, abnormalIdx, normalIdx, field_names, exp_param.model_name);
            if exp_param.llm_only
                predicates = llm_predicates;  % Use only LLM rules
                % Skip the rest of predicate generation
                goto_evaluation = true;
            else
    ...
    if ~exist('goto_evaluation', 'var')
        % Original predicate generation code here
        predicates = {};
        % ... (rest of predicate generation)
    end
    ...
    % Then proceed with evaluation using the updated predicates
